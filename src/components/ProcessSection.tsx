  ...
  return (
    <section id="process" className="py-16 sm:py-32 px-4 sm:px-6 relative overflow-hidden" ref={ref}>
      ...
      <div className="absolute inset-0 z-0">
        <img src={processBg} alt="" className="absolute inset-0 w-full h-full object-cover blur-sm" loading="lazy" width={1920} height={1080} />
      ...
      <div className="container mx-auto max-w-5xl relative z-10">
        ...
        <div className="relative">
          <div className="absolute left-6 sm:left-8 md:left-1/2 top-0 bottom-0 w-px bg-border md:-translate-x-px" />
          ...
          {steps.map((step, i) => (
            ...
            >
              <motion.div
                className="absolute left-6 sm:left-8 md:left-1/2 w-3 h-3 rounded-full bg-primary -translate-x-1.5 z-10"
                initial={{ scale: 0 }}
                animate={inView ? { scale: 1 } : {}}
                transition={{ duration: 0.4, delay: i * 0.15 + 0.3 }}
              />
              <motion.div
                className={`ml-14 sm:ml-20 md:ml-0 md:w-1/2 ${
                  i % 2 === 0 ? "md:pr-16 md:text-right" : "md:pl-16"
                }`}
                whileHover={{ scale: 1.02 }}
                transition={{ duration: 0.2 }}
              >
                <span className="text-primary/40 font-display text-2xl sm:text-4xl font-bold">
                  {step.number}
                </span>
                <h3 className="font-display text-xl sm:text-2xl font-semibold mt-1 mb-1 sm:mb-2">
                  {step.title}
                </h3>
