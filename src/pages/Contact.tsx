  ...
  const contactItems = [
    {
      ...
      content: (
        <a
          href={`mailto:${c.email}`}
          className="inline-block bg-gold-gradient text-primary-foreground px-4 sm:px-6 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold tracking-wider uppercase rounded-sm hover:opacity-90 hover:scale-[1.03] transition-all duration-300 font-body"
        >
          Send Email
        </a>
      ),
      ...
      <section className="pt-24 sm:pt-32 pb-12 sm:pb-20 px-4 sm:px-6" ref={ref}>
        <div className="container mx-auto max-w-4xl">
          ...
          >
            ...
            <p className="text-muted-foreground font-body text-sm sm:text-lg mt-4 sm:mt-6 max-w-xl mx-auto">
              {c.description}
            </p>
          </motion.div>
          ...
          >
            {contactItems.map((item) => (
              ...
              >
                <motion.div
                  className="w-14 sm:w-16 h-14 sm:h-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto"
