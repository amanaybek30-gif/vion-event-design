    ...
    <div className="min-h-screen bg-secondary text-secondary-foreground">
      ...
      <Section className="relative pt-32 pb-24 px-6">
        ...
        <div className="container mx-auto max-w-4xl text-center relative z-10">
          <motion.p variants={fadeUp} transition={{ duration: 0.6 }} className="font-body text-xs tracking-[0.3em] uppercase text-primary mb-6">
            {c.hero_subtitle}
          </motion.p>
          <motion.h1 variants={fadeUp} transition={{ duration: 0.6, delay: 0.05 }} className="font-display text-4xl md:text-6xl lg:text-7xl font-bold leading-[0.95] mb-6" style={{ lineHeight: 1.05 }}>
            {c.hero_title_start}{" "}<span className="text-gold-gradient">{c.hero_title_highlight}</span>
          </motion.h1>
          <motion.p variants={fadeUp} transition={{ duration: 0.6, delay: 0.1 }} className="font-body text-base md:text-lg max-w-2xl mx-auto mb-10 text-white/70">
            {c.hero_description}
          </motion.p>
          <motion.div variants={fadeUp} transition={{ duration: 0.6, delay: 0.15 }} className="flex flex-col sm:flex-row gap-4 justify-center">
      ...
      <Section className="py-24 px-6 section-dark">
        <div className="container mx-auto max-w-6xl">
          <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="font-display text-3xl md:text-4xl font-bold text-center mb-4" style={{ lineHeight: 1.1 }}>
            {c.features_title_start}{" "}<span className="text-gold-gradient">{c.features_title_highlight}</span>
          </motion.h2>
          <motion.p variants={fadeUp} transition={{ duration: 0.6, delay: 0.05 }} className="font-body text-sm text-center max-w-xl mx-auto mb-16 text-white/60">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilities.map((cap, i) => (
              <motion.div key={cap.title} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.07 }}
                ...
                <cap.icon size={28} className="text-primary mb-4 group-hover:scale-105 transition-transform duration-300" strokeWidth={1.5} />
      ...
      <Section className="py-28 px-6">
        <div className="container mx-auto max-w-4xl">
          <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="font-display text-3xl md:text-4xl font-bold text-center mb-16" style={{ lineHeight: 1.1 }}>
            {c.how_title_start} <span className="text-gold-gradient">{c.how_title_highlight}</span>
          </motion.h2>
          <div className="grid md:grid-cols-2 gap-12">
            <motion.div variants={fadeUp} transition={{ duration: 0.5 }}>
              <h3 className="font-display text-xl font-semibold mb-6 text-primary">For Attendees</h3>
              <ol className="space-y-4">
                {["Browse events on the VION website", "Click Register — powered by VERS", "Complete registration & receive QR code", "Use QR code for instant check-in"].map((step, i) => (
                  <li key={i} className="flex gap-4 items-start">
                    <span className="font-display text-2xl font-bold text-primary/30 leading-none mt-0.5">{String(i + 1).padStart(2, "0")}</span>
            ...
            <motion.div variants={fadeUp} transition={{ duration: 0.5, delay: 0.08 }}>
              <h3 className="font-display text-xl font-semibold mb-6 text-primary">For Organizers</h3>
              <ol className="space-y-4">
      ...
      <Section className="py-28 px-6">
        <div className="container mx-auto max-w-3xl text-center">
          <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="font-display text-3xl md:text-5xl font-bold mb-6" style={{ lineHeight: 1.05 }}>
            {c.cta_title_start}{" "}<span className="text-gold-gradient">{c.cta_title_highlight}</span>
          </motion.h2>
          <motion.p variants={fadeUp} transition={{ duration: 0.6, delay: 0.05 }} className="font-body text-base mb-10 text-white/70">
            {c.cta_description}
          </motion.p>
          <motion.div variants={fadeUp} transition={{ duration: 0.6, delay: 0.1 }} className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="https://vers.vionevents.com" target="_blank" rel="noopener noreferrer"
